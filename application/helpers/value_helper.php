...
if (!function_exists('values')) {
    ...
    {
        if (is_null($value) || empty($value) || $value == 'undefined' || $value == 'null') {
            return $default;
        }
...
if (!function_exists('get_week_date_range')) {
    function get_week_date_range($week, $year) {
        $dto = new DateTime();
        $ret['week_start'] = $dto->setISODate($year, $week)->modify('-1 day')->format('Y-m-d');
...
if (!function_exists('invert_chat_id')) {
    /**
     * Normalize chat id.
     *
     * @param $chatId
     * @return string
     */
    function invert_chat_id($chatId, $code = false)
    {
        $chatId = str_replace(['@c.us'], '', $chatId);
        if(!$code){
            $chatId = preg_replace('/^628/', '08', $chatId);
        }
...
if (!function_exists('mid_ellipsis')) {
     ...
     * @return array|string
     */
    function mid_ellipsis($value, $length = 4, $start = 4)
    {
        return substr_replace($value, '...', $start, strlen($value) - ($length * 2));
    }
}